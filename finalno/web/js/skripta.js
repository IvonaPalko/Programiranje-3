...
function ucitaj(){
	$.getJSON(putanjaAPI + "/automobili", function( jsonData ) {
...
function definirajDogadajeNakonAJAXa(){
	...
	$(".promjena").click(function(){
		...
		$.getJSON(putanjaAPI + "/automobili/" + sifra, function( jsonData ) {
...
function postaviNaTablicu(jsonData){
	$("#podaci").html("");
	  $.each( jsonData, function( kljuc, operater ) {
	    ...
	    "<td>" + operater.naziv + "</td>" +
	    "<td>" + operater.proizvodjac + "</td>" +
